...
def get_user_by_id(user_id: int) -> Dict[str, Any]:
    ...
    for user in data["Users"]:
        if user["id"] == user_id:
            return user
    raise ValueError(f"Nem található felhasználó a megadott ID-val: {user_id}")
...
def get_basket_by_user_id(user_id: int) -> List[Dict[str, Any]]:
    ...
    for basket in data["Baskets"]:
        if basket["user_id"] == user_id:
            return basket["items"]
    raise ValueError(f"Nem található kosár a megadott felhasználó ID-val: {user_id}")
...
def get_total_price_of_basket(user_id: int) -> float:
    ...
    for basket in data["Baskets"]:
        if basket["user_id"] == user_id:
            total_price = sum(
                item["price"] * item["quantity"] for item in basket["items"]
            )
            return total_price
    raise ValueError(f"Nem található kosár a megadott felhasználó ID-val: {user_id}")
