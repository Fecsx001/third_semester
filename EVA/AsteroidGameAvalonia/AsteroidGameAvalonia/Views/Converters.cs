...
namespace AsteroidGameAvalonia.Views
{
    public class HeightConverter : IValueConverter
    {
        ...
        {
            if (value is int height) return height + Offset;
            if (value is double dHeight) return dHeight + Offset;
            return value;
        }
    ...
    public class AsteroidStrokeConverter : IValueConverter
    {
        public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            if (widthProperty != null && heightProperty != null)
            {
                ...
                if (size >= 60) return Brushes.DarkRed; 
                if (size >= 40) return Brushes.DarkOrange; 
                if (size >= 25) return Brushes.White; 
